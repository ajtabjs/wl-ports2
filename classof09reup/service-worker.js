var cacheName = 'class-of-09-the-re-up';
...
async function fetchAndCache(request) {
    ...
    try {
        ...
        // Split media files (for example .webm.partN) are reconstructed and
        // cached by the page script. If the original URL is missing on the
        // network (404/other non-OK), prefer the cached reconstructed version.
        if (cachedResponse && !response.ok) {
            console.log('Network fetch failed, served from cache: ' + request.url);
            return cachedResponse;
        }
